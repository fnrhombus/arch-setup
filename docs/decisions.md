...
- **Battery**: NONE *currently* — the internal battery is dead / removed; user plans to replace it. Laptop is always on AC power, lives stashed under a desk. Downstream consequences:
  - Lid-close hibernate is owned by `~/.local/bin/lid-handler` from the user's Hyprland session (logind defers across the board — see Lid-close Requirement). The automatic headless-on-battery path is live the moment a battery returns; no reconfiguration needed.
  - **Hibernation is enabled** (S4). Until the battery is replaced, hibernate is **user-invoked** (`Super+Shift+H`) since AC removal is an instant hard-cut. The automatic path (headless + battery, via `/usr/local/bin/headless-hibernate`) also exists and fires on lid close, AC unplug, or display hotplug once a battery is present. Swap sized 16 GiB to match RAM, lives as a NoCOW swapfile inside the LUKS-encrypted btrfs root (subvolume `@swap`) — see §Q9 + §Q11. See `docs/desktop-requirements.md` §Hibernate for the full plan.
...
- [x] Lid close: hibernate iff lid closed AND no external display connected AND on battery. External-display detection keys on connector *status* (`/sys/class/drm/*/status`), not DPMS state — an idle-blanked screen still counts as connected. All lid events are dispatched via `~/.local/bin/lid-handler` ([rhombu5/dots](https://github.com/rhombu5/dots)), wired via `binddl` on `Lid Switch` in `dot_config/hypr/binds.conf`. On lid close, lid-handler disables `eDP-1` if an external monitor is attached (re-enabled on lid open), then delegates the hibernate decision to `/usr/local/bin/headless-hibernate`. That script is the single source of truth for the three-condition rule; it is also invoked by a udev-triggered oneshot service (`headless-hibernate-check.service`) on AC plug/unplug and display hotplug events (rule: `/etc/udev/rules.d/99-headless-hibernate.rules`). Logind defers entirely (`Handle*=ignore` everywhere, written by phase-2 `chroot.sh` to `/etc/systemd/logind.conf.d/10-lid.conf`); at the greeter / TTY (no Hyprland) lid close is a no-op, which is fine since those states only exist with the laptop physically open. Companion daemon `~/.local/bin/display-watchdog` ([rhombu5/dots](https://github.com/rhombu5/dots)) handles display-recovery only: re-enables `eDP-1` on lid-open blackout (last external disconnects while `eDP-1` is disabled and lid is open).
...
- **Update 2026-06-04 — WinApps retargeted to physical hardware**: the VM's 8G resident QEMU process was the standing memory pressure behind a systemd-oomd kill of the entire Hyprland session (metis has 16G; oomd killed the compositor cgroup at 95% mem / 85% swap). WinApps now points at the physical Windows 10 Pro box `callisto.rhombus.rocks` (`WAFLAVOR=manual`, MS-account login, user + password filled by the `winapps-rdp-pass.sh` planter from Bitwarden — postinstall writes both empty so no account PII lands in this public repo). The VM stays fully installed as a dormant fallback — `restart: "no"`, never auto-starts; `sudo docker start WinApps` + repoint `winapps.conf` at `127.0.0.1` to use it (e.g. when callisto is unreachable). Callisto prerequisite (reapply on its reinstall): `fAllowUnlistedRemotePrograms=1` under `HKLM\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services`.
...
`root=/dev/mapper/cryptroot rootflags=subvol=@ resume=/dev/mapper/cryptroot resume_offset=<N> rw quiet video=DP-1:3840x2160@60`. The `<N>` is the swapfile's physical extent offset on the btrfs volume, captured by `install.sh` §8.5 via `btrfs inspect-internal map-swapfile -r`. No `rd.luks.name=` needed — crypttab.initramfs is the single source of truth. The `video=DP-1:3840x2160@60` hint forces native-res console on the 4K TV (without it, the kernel framebuffer comes up at 1080p in the top-left quarter); `@60` matches what Hyprland negotiates so fb0 doesn't fall back to 1080p when Hyprland releases the DRM master on chvt. Per-connector keying means eDP-1 is unaffected.

**Secure Boot readiness:**
- Secure Boot stays **off** at install time. Install pre-stages everything for a one-trip enable: `sbctl` + `efitools` are pacstrap'd; `chroot.sh` runs `sbctl create-keys`, signs all four EFI binaries (limine fallback + source, both UKIs), and exports PK/KEK/db as UEFI `.auth` files at `/boot/EFI/sbctl-keys/`. `95-limine-redeploy.hook` and sbctl's own `zz-sbctl.hook` keep signatures fresh across upgrades.
- Enabling SB later: one BIOS trip — Expert Key Management → Custom Mode on → Replace PK from `EFI/sbctl-keys/PK.auth` → Append KEK from `EFI/sbctl-keys/KEK.auth` → Append db from `EFI/sbctl-keys/db.auth` → Secure Boot Enable on → save & exit. First boot prompts for the recovery key once (PCR 7 changed); `sudo /usr/local/sbin/tpm2-reseal-luks` rebinds the seal to PCR 7=SB-on; subsequent boots silent. Full procedure in `runbook/phase-3-handoff.md` "Upgrade Paths → Secure Boot via sbctl".
