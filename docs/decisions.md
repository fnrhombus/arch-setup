...
- [x] Lid close on AC: hibernate if no external monitor is attached, otherwise disable `eDP-1` (re-enabled on lid open). Logind hands off the AC branch (`HandleLidSwitchExternalPower=ignore`, `HandleLidSwitchDocked=ignore`, written by phase-2 `chroot.sh` to `/etc/systemd/logind.conf.d/10-lid.conf`); the conditional logic lives in `~/.local/bin/lid-handler` ([rhombu5/dots](https://github.com/rhombu5/dots)) wired via `binddl` on `Lid Switch` in `dot_config/hypr/binds.conf`.
