...
- [assets/Win11_25H2_English_x64_v2.iso.sha256](assets/Win11_25H2_English_x64_v2.iso.sha256) — **Manually-fetched** sidecar holding the official SHA256 for the Win11 25H2 consumer ISO, copied from microsoft.com/software-download/windows11 (commit 19ed2e4). Microsoft doesn't ship this hash in any auto-fetchable feed, and Fido doesn't return one — so the value lives in git as the source-of-truth that fetch-assets.ps1 verifies the download against and stage-ventoy.ps1 verifies the on-medium copy against. **Bump together with the canonical ISO filename** when Microsoft ships 26H2.
