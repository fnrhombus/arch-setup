...
- [phase-2-arch-install/chroot.sh](phase-2-arch-install/chroot.sh) — Inside `arch-chroot /mnt`. Consumes the pre-hashed passwords, sets timezone + locale, creates user `tom`, allocates TPM2 SHA-256 PCR bank, installs **limine** (UEFI binary to ESP fallback path + NVRAM entry + `/boot/limine.conf` chainloading the UKIs), configures mkinitcpio in UKI mode + `/etc/kernel/uki.conf` (signed-PCR-11) + `/etc/kernel/cmdline` (with `resume_offset=`), installs **greetd + greetd-regreet** (system-files at `phase-3-arch-postinstall/system-files/{greetd,pam.d}/` — greetd is enabled here but postinstall §1f disables it in favour of bare TTY login; packages stay as a recoverable fallback), writes `/etc/crypttab.initramfs` (single cryptroot entry), installs `/usr/local/sbin/tpm2-reseal-luks` as a manual recovery tool, wires greetd PAM for gnome-keyring + fprintd (kept current for the fallback case), seeds Wi-Fi profiles, enables services.
...
- **`yay -S` invocations pass `--rmdeps`.** Without it, yay leaves transitive build dependencies (compilers, language runtimes) installed system-wide as orphans — which silently violates the global "mise for devtools" rule from `~/.claude/CLAUDE.md`. With `--rmdeps`, makepkg-installed deps are removed after the build completes, leaving only the actual runtime dependencies. Apply to one-off installs *and* anything you add to `phase-3-arch-postinstall/postinstall.sh`'s yay loop. (Discovered 2026-05-11 after `rust`, `go`, `npm`, `electron`, `gradle` and others were found orphaned from earlier AUR builds.)
