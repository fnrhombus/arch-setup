...
- [scripts/fetch-win11-hash.mjs](scripts/fetch-win11-hash.mjs) — Playwright-driven scraper that opens the official Microsoft Windows 11 download page, selects multi-edition x64 + English, clicks "Verify your download", and extracts the SHA-256 from the revealed hash table. Two pnpm entry points: `pnpm hash:win11` (print only) and `pnpm hash:win11:update` (overwrites the in-git sidecar). fetch-assets.ps1's soft-warn message points the user here when the in-git sidecar drifts. The hash sits behind a JS-rendered link, so static HTML scraping doesn't see it — Playwright is the only practical way to automate this.
