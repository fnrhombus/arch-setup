...
- [scripts/fetch-assets.ps1](scripts/fetch-assets.ps1) — Downloads the Arch ISO (latest from Rackspace mirror) + Ventoy Windows release (latest from GitHub API) + Windows 11 consumer ISO via vendored [Fido.ps1](https://github.com/pbatard/Fido) into `assets/`. Fido output is renamed to the canonical `Win11_25H2_English_x64_v2.iso` so `ventoy/ventoy.json`'s `auto_install` match keeps working — bump both together when Microsoft ships 26H2. Idempotent (`-Force` to override). On any Fido failure (MS API drift, etc.) falls through to actionable manual-download instructions. Verifies the downloaded Win11 ISO against `assets/Win11_25H2_English_x64_v2.iso.sha256` (next bullet).
- [assets/Win11_25H2_English_x64_v2.iso.sha256](assets/Win11_25H2_English_x64_v2.iso.sha256) — **Authoritative** sidecar holding the official SHA-256 for the Win11 25H2 consumer ISO. Original value (commit 19ed2e4) was hand-fetched from microsoft.com/software-download/windows11; subsequent updates run via `pnpm hash:win11:update` (next bullet). Microsoft doesn't ship this hash in any auto-fetchable feed, and Fido doesn't return one — so the value lives in git as the source-of-truth that fetch-assets.ps1 verifies the download against and stage-ventoy.ps1 verifies the on-medium copy against. **Bump together with the canonical ISO filename** when Microsoft ships 26H2.
- [scripts/fetch-win11-hash.mjs](scripts/fetch-win11-hash.mjs) — Playwright-driven scraper that opens the official Microsoft Windows 11 download page, selects multi-edition x64 + English, clicks "Verify your download", and extracts the SHA-256 from the revealed hash table. Optionally downloads the ISO from the same browser session (the download URL is one-shot session-bound, hence captured-and-clicked rather than fetched standalone). Three pnpm entry points: `pnpm hash:win11` (print hash only), `pnpm hash:win11:update` (overwrites the in-git sidecar), `pnpm fetch:win11` (downloads ISO, verifies against scraped hash, updates sidecar — the canonical "from-scratch Win11 acquisition" path that supersedes Fido). fetch-assets.ps1's soft-warn message points the user here when the in-git sidecar drifts. Why Playwright instead of curl: the hash sits behind a JS-rendered link AND the download URL is generated per-session — neither survives static HTML scraping. Run with `--debug` for a non-headless browser when selectors need re-validating.
