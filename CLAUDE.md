...
- [scripts/fetch-assets.ps1](scripts/fetch-assets.ps1) — Downloads the Arch ISO (latest from Rackspace mirror) + Ventoy Windows release (latest from GitHub API) + Windows 11 25H2 consumer multi-edition ISO via vendored [Fido.ps1](https://github.com/pbatard/Fido) (pinned to `-Rel '25H2'` so MS rolling won't surprise us; multi-edition contains Pro which `autounattend.xml` selects from `install.wim`). Fido output is renamed to the canonical `Win11_25H2_English_x64_v2.iso` so `ventoy/ventoy.json`'s `auto_install` glob keeps working — bump the canonical filename + ventoy.json + the Fido `-Rel` pin together when MS ships 26H2. Idempotent (`-Force` re-downloads). Verifies the Arch ISO against `archlinux-sha256sums.txt`; **does NOT verify a Win11 hash** — MS doesn't ship one in any auto-fetchable feed, Fido doesn't return one, and the brief Playwright workaround we tried got the user's IP soft-banned from the MS download site. Trust Fido.
