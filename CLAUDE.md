...
- [package.json](package.json) — Entry points for the dev-machine prep flow. `pnpm i` chains `fetch-assets.ps1` → `stage-ventoy.ps1` (postinstall hook). `pnpm stage` re-runs the Ventoy staging only; `pnpm restore:force` re-downloads ISOs. `pnpm pdf` renders every `runbook/*.md` to a sibling `.pdf`.
- [scripts/fetch-assets.ps1](scripts/fetch-assets.ps1) — Downloads the Arch ISO (latest from Rackspace mirror) + Ventoy Windows release (latest from GitHub API) + Windows 11 consumer ISO into `assets/`. The Win11 path delegates to [scripts/fetch-win11-hash.mjs](scripts/fetch-win11-hash.mjs) (next bullets) since Fido was retired 2026-04-27. Canonical Win11 filename `Win11_25H2_English_x64_v2.iso` matches `ventoy/ventoy.json`'s `auto_install` glob — bump both together when Microsoft ships 26H2. Idempotent (`-Force` re-downloads). Verifies both ISOs against their sidecars after copy.
- [scripts/fetch-win11-hash.mjs](scripts/fetch-win11-hash.mjs) — Playwright-driven script that drives the official Microsoft Windows 11 download page (selects multi-edition x64 + English, clicks "Verify your download"), extracting the authoritative SHA-256 AND grabbing the per-session ISO URL from the same browser context. Three pnpm entry points: `pnpm hash:win11` (print hash only), `pnpm hash:win11:update` (write sidecar), `pnpm fetch:win11` (download ISO + verify + write sidecar — the canonical from-scratch Win11 acquisition). fetch-assets.ps1 calls `pnpm fetch:win11` automatically when the ISO is missing. Why Playwright instead of curl: the hash sits behind a JS-rendered link AND the download URL is per-session — neither survives static scraping. Run with `--debug` for a non-headless browser when selectors need re-validating. **Replaced Fido** ([pbatard/Fido](https://github.com/pbatard/Fido)) because Fido provides no hash retrieval API, leaving fresh downloads un-reconcilable with our pinned in-git sidecar.
- [assets/Win11_25H2_English_x64_v2.iso.sha256](assets/Win11_25H2_English_x64_v2.iso.sha256) — Authoritative sidecar holding the official SHA-256 for the Win11 25H2 consumer ISO. Originally hand-fetched (commit 19ed2e4); subsequent refreshes run via `pnpm hash:win11:update` (or auto-written by `pnpm fetch:win11`). Microsoft doesn't ship this hash in any auto-fetchable feed, so the value lives in git as the source-of-truth that fetch-assets.ps1 + stage-ventoy.ps1 verify against. **Bump together with the canonical ISO filename** when Microsoft ships 26H2.
- [scripts/stage-ventoy.ps1](scripts/stage-ventoy.ps1) — Auto-finds the Ventoy data partition by its `Ventoy` filesystem label, sanity-checks with the ~32 MB VTOYEFI companion, then mirrors ISOs + configs + phase scripts + `docs/` + `runbook/` onto it via robocopy. Post-copy SHA256-verifies BOTH the Arch ISO (against the `archlinux-sha256sums.txt` mirror file) and the Win11 ISO (against the manually-fetched sidecar). Soft-exits (code 2) if no Ventoy medium is present, so `pnpm i` never fails just because nothing's plugged in. Selects internal Netac-Ventoy (Phase 0-alt) by the same label match — drive-agnostic.
...
- [ventoy/ventoy.json](ventoy/ventoy.json) — Ventoy plugin config. Makes Ventoy inject `autounattend.xml` when the Win11 ISO is selected from the menu (otherwise Ventoy's Windows-installer emulation ignores the sibling XML). Mirrored to `<Ventoy-data>/ventoy/ventoy.json` by `stage-ventoy.ps1`.
