...
- Add bindings to `dot_config/hypr/binds.conf` in [rhombu5/dots](https://github.com/rhombu5/dots) using the new `gesture = ...` syntax (Hyprland 0.51+) — three-finger swipe left/right → workspace prev/next, three-finger up → Hyprspace overview (`overview:toggle`). Edit via `chezmoi edit ~/.config/hypr/binds.conf` then `chezmoi cd` to commit + push to dots.
