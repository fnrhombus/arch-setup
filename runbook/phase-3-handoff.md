...
- Integrate with: TTY login (`/etc/pam.d/login` — `pam_fprintd_grosshack` races finger vs typed input; `pam_unix` tests typed as password; PIN excluded by design), sudo, screen lock (hyprlock), polkit-1 — the in-session surfaces also accept PIN via `libpinpam`.
