...
- **systemd-cryptenroll** — Binds a LUKS2 key slot to a TPM2 (or FIDO2 / PKCS#11) credential. We enroll BOTH `ArchRoot` and `ArchSwap` (per postinstall.sh §7.5). Manual rebind via `/usr/local/sbin/tpm2-reseal-luks` on PCR 7 events (BIOS update / Secure Boot toggle / TPM clear); kernel/UKI updates need no rebind because the UKI ships its own `.pcrsig`.
...
- **PAM** (Pluggable Authentication Modules) — Linux's auth stack. TTY login, sudo, hyprlock, polkit-1 all go through `/etc/pam.d/*`. Two surface flavors (per postinstall.sh §7a): in-session re-auth (sudo / hyprlock / polkit-1) runs `pam_fprintd_grosshack` + `libpinpam.so` + `pam_unix.so` concurrently — finger, PIN, or password. Cold-boot TTY login (`/etc/pam.d/login`) runs only `pam_fprintd_grosshack` + `pam_unix.so` — PIN is NOT a login factor by design.
- **pinpam / libpinpam.so** — TPM-backed PIN auth module. We ship a fork (`pinpam-fnrhombus`, built from `phase-3-arch-postinstall/aur-overrides/pinpam-fnrhombus/PKGBUILD`) carrying a `try_first_pass` / `use_first_pass` patch until upstream merges. The PAM module name is literally `libpinpam.so` (NOT `pam_pinpam.so`); referencing the wrong name silently dlopen-fails. PIN itself is set up via `pinutil setup`.
- **pam-fprint-grosshack** — PAM module that races fingerprint verification (via fprintd D-Bus) against a typed-input pthread. We ship a fork (`pam-fprint-grosshack-fnrhombus`) with a 1-line patch that resets a static SIGUSR1 flag at the start of each `pam_authenticate` call — without it, sudo's retry loop is broken (typed input leaks into pam_unix's prompt). Upstream is functionally abandoned (last commit 2022-07-27).
- **physlock** — TTY-based screen lock. On `loginctl lock-session` (Super+Ctrl+L → logind → hypridle → physlock), it chvts to a free TTY, blocks all VT switching, runs PAM auth there, and chvts back on unlock. Hyprland session is preserved untouched on its own VT. Replaced hyprlock as the lock_cmd 2026-05-05 — same PAM stack (in-session re-auth: finger / PIN / password) but no graphical-lockscreen dependency. AUR package; `/etc/pam.d/physlock` includes the hyprlock stack.
- **claude-askpass** / **sudoa** — `~/.local/bin/claude-askpass` (chezmoi-managed in dots) reads tom's local password from a Bitwarden vault item and prints it to stdout, used as `SUDO_ASKPASS`. The `sudoa` zsh function (also dots) is `SUDO_ASKPASS=… sudo -A …` — unattended sudo for batches Claude can't drive interactively. Postinstall §0 captures the password once and runs hands-off after that — four paths, in priority order: `$PASS` env var (automation escape hatch), pre-set `SUDO_ASKPASS`, `~/.local/bin/claude-askpass` (subsequent runs), interactive `read -rs` prompt (fresh installs). All paths converge on a tmpfs-backed `SUDO_ASKPASS` so the keeper survives any installer mid-run that clears sudo's timestamp. No `sudo -v` prereq.
