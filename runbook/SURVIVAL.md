...
If there's no response at all — the kernel or firmware wedged. Hold power **10 seconds**, power back on, at the **limine boot menu** pick a different entry under `Arch Linux`:
  - "UKI (linux-lts)" — known-good older kernel for kernel-regression bugs (TPM-unsealed, silent boot).
  - "Recovery (linux)" — bare-kernel boot path, **prompts for the LUKS passphrase** (the recovery path doesn't go through the TPM seal — that's by design, the `.pcrsig` is UKI-specific).
  - **Snapshot entries under the `Snapshots` directory** (auto-generated by `limine-snapper-sync`) — pick yesterday's snapshot to roll back a bad `pacman -Syu` without entering chroot rescue. These also prompt for the LUKS passphrase since they clone the Recovery (linux) entry.
